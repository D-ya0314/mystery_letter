...
exports.handler = async (event, context) => {
  ...
  /* ★ここから追加：特別権限用のホワイトリスト機能★ */
  // const ALLOWED_USERS = [
  //   "U0602079f1acff8298d2bc73fa2e0b6d5", // あなたのLINEユーザーID
  //   "Uyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy", // 家族のLINEユーザーID
  // ];
  const allowedString = process.env.ALLOWED_USERS_LIST || "";
  const ALLOWED_USERS = allowedString.split(",");
  ...
  try {
    ...
    const response = await fetch(
      "https://line.me/v2/bot/membership/user/" + userId,
      {
        method: "GET",
        headers: {
